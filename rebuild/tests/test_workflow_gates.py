...
def test_release_publish_requires_every_safety_gate() -> None:
    ...
    required = (
        "detect-arch-release", "quality", "shell", "contracts", "first-login-pty", "build-iso",
        "build-windows-exe", "offline-iso-boot", "vm-install-reboot", "recovery-rehearsal",
    )
    needs_line = next(line for line in publish.splitlines() if "needs:" in line)
    assert all(job in needs_line for job in required)
    assert "needs.detect-arch-release.outputs.should_run == 'true'" in publish
    assert "needs.detect-arch-release.outputs.publish == 'true'" in publish
    ...
    assert "test -e /dev/kvm" in workflow
