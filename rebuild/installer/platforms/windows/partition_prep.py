...
DEFAULT_MIN_SHRINK_BYTES = 20 * 1024 * 1024 * 1024
...
class PartitionPrepPolicy:
    ...
    minimum_shrink_bytes: int = DEFAULT_MIN_SHRINK_BYTES
    dry_run: bool = False

    def __post_init__(self) -> None:
        if self.target_free_space_bytes <= 0:
            raise ValueError("target_free_space_bytes must be greater than zero.")
        if self.minimum_shrink_bytes < 0:
            raise ValueError("minimum_shrink_bytes must be zero or greater.")
...
def _calculate_shrink_bytes(current_free_space_bytes: int, policy: PartitionPrepPolicy) -> int:
    ...
    if missing <= 0:
        return 0
    return max(missing, policy.minimum_shrink_bytes)
