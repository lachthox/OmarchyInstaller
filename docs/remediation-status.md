...
| HIGH-18 | High | EFI mount contracts conflict | 12, 13, 15 | install/guardian | Engine and machine-specific guardian state use the plan's sole `/boot` ESP contract | command plan and finalization state assert canonical mount | Open | | Final boot/reboot verification remains Phase 15 |
...
| HIGH-29 | High | Guardian defaults when expected state absent | 13, 15 | guardian | Finalization writes and validates actual machine GUID/UUID/ESP state before enabling guardian | missing/invalid expected-state prevents activation and success marker | Open | | Phase 15 removes remaining runtime fallback |
